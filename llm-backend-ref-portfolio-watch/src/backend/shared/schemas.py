...
from backend.domain.entities import EventRoute, RoutingDecision, Severity, SeverityLevel
from backend.infra.llm.structured import parse_structured
...
class RewriteOutput(BaseModel):
    ...
    intent: Literal["price_lookup", "news_lookup", "explain", "diagram", "chart"] = Field(
    ...
    def _clean_intent(cls, v: Any) -> str:
        ...
        if s in {"price_lookup", "news_lookup", "explain", "diagram", "chart"}:
            return s
        return "price_lookup"
...
class SupervisorOutput(BaseModel):
    ...
    agents_to_call: list[Literal["price", "news", "eval", "diagram", "chart"]] = Field(
        default_factory=lambda: ["price"],
        description="Danh sách agents cần gọi (price, news, eval, diagram, chart)",
    )
    reason: str = Field(
        default="",
    ...
    def _clean_agents(cls, v: Any) -> list[str]:
        if not isinstance(v, list):
            return ["price"]
        allowed = {"price", "news", "eval", "diagram", "chart"}
        valid: list[str] = []
        for a in v:
