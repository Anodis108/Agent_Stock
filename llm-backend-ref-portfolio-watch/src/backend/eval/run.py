...
Injection gate: slice injection phải 100% pass — không tolerance.
    ...
    python -m backend.eval.run --run --skip-judge --limit 3
    docker compose run --rm ai python -m backend.eval.run --run --save-baseline
"""
...
GOLDEN_PATH = (
    ROOT / "resources" / "eval" / "golden_dataset.yaml"
    if (ROOT / "resources" / "eval" / "golden_dataset.yaml").is_file()
    else ROOT / "specs" / "eval" / "golden_dataset.yaml"
)
GOLDEN_V3_PATH = (
    ROOT / "resources" / "eval" / "golden_v3.yaml"
    if (ROOT / "resources" / "eval" / "golden_v3.yaml").is_file()
    else ROOT / "specs" / "eval" / "golden_v3.yaml"
)
GOLDEN_V4_PATH = (
    ROOT / "resources" / "eval" / "golden_v4.yaml"
    if (ROOT / "resources" / "eval" / "golden_v4.yaml").is_file()
    else ROOT / "specs" / "eval" / "golden_v4.yaml"
)
GOLDEN_V5_PATH = (
...
BLOCKED_PATH = (
    ROOT / "resources" / "eval" / "blocked_cases.yaml"
    if (ROOT / "resources" / "eval" / "blocked_cases.yaml").is_file()
    else ROOT / "specs" / "eval" / "blocked_cases.yaml"
)
...
from backend.domain.guardrails.output_checks import (  # noqa: E402
    find_buy_sell_phrases,
)
from backend.infra.eval.agent_scorers import (  # noqa: E402
    TASK_SUCCESS_SLICES,
...
from backend.infra.llm.completion import chat_parsed  # noqa: E402
from backend.infra.llm.params import DETERMINISTIC  # noqa: E402
...
class LlmJudgeScore(BaseModel):
    ...
    correctness: int = Field(ge=1, le=5, description="Điểm số thang 1-5 (5 là hoàn toàn chính xác, 1 là hoàn toàn sai)")
    completeness: int = Field(ge=1, le=5, description="Điểm số thang 1-5 (5 là trả lời đầy đủ, 1 là thiếu ý)")
    grounding: int = Field(
        ge=1, le=5, description="Điểm số thang 1-5 (5 là hoàn toàn bám sát căn cứ/expected, 1 là bịa đặt)"
    )
