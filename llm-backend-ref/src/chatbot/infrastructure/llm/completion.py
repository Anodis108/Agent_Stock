...
def chat(messages: Messages, params: GenerationParams | None = None) -> str:
    ...
    response = retry_with_backoff( # đầu tiền cứ thực hiện back_off
        _call,
        max_retries=settings.llm_max_retries,
        on_rate_limit=lambda: mark_current_key_limited(client), # chỉ rotting key khi gặp 429 và on rate_limit
    )
